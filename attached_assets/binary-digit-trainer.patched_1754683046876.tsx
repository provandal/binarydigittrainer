...
function handleExportCheckpoint() {
  const cp: Checkpoint = {
    ...
    normalize: { enabled: normalizeEnabled ?? true, targetSize: targetSize ?? 7 },
    ...
    params: {
      weights: currentNetworkState.current.weights.map((r: number[]) => [...r]),
      biases:  [...currentNetworkState.current.biases],
      outputWeights: currentNetworkState.current.outputWeights.map((r: number[]) => [...r]),
      outputBiases:  [...currentNetworkState.current.outputBiases]
    }
  };
...
async function handleImportCheckpointFile(e: React.ChangeEvent<HTMLInputElement>) {
  ...
  try {
    ...
    if (normalize && typeof normalize.enabled === "boolean") setNormalizeEnabled(!!normalize.enabled);
    if (normalize && typeof normalize.targetSize === "number") setTargetSize(normalize.targetSize);
